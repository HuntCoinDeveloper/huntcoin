...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction &transaction, const CCoinControl& coinControl)
{
    ...
    for (const SendCoinsRecipient &rcp : recipients)
    {
        ...
        {   // User-entered huntcoin address / amount:
...

    {
        ...
        if(recipients[0].fUseInstantSend && total > sporkManager.GetSporkValue(SPORK_3_INSTANTSEND_MAX_VALUE)*COIN){
            Q_EMIT message(tr("Send Coins"), tr("InstantSend doesn't support sending values that high yet. Transactions are currently limited to %1 HUNT.").arg(sporkManager.GetSporkValue(SPORK_3_INSTANTSEND_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction &transaction)
{
...

    {
        ...
        for (const SendCoinsRecipient &rcp : recipients)
        {
            ...
            else if (!rcp.message.isEmpty()) // Message from normal huntcoin:URI (huntcoin:123...?message=example)
...
bool WalletModel::bumpFee(uint256 hash)
{
    ...
    questionString.append(HuntcoinUnits::formatHtmlWithUnit(getOptionsModel()->getDisplayUnit(), old_fee));
    questionString.append("</td></tr><tr><td>");
    questionString.append(tr("Increase:"));
    questionString.append("</td><td>");
    questionString.append(HuntcoinUnits::formatHtmlWithUnit(getOptionsModel()->getDisplayUnit(), new_fee - old_fee));
    questionString.append("</td></tr><tr><td>");
    questionString.append(tr("New fee:"));
    questionString.append("</td><td>");
    questionString.append(HuntcoinUnits::formatHtmlWithUnit(getOptionsModel()->getDisplayUnit(), new_fee));
