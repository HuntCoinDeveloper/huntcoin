    ...
    platformStyle(_platformStyle)
{
    ...
    // see https://github.com/HuntCoinDeveloper/huntcoin/issues/5716
...
{
    GUIUtil::setClipboard(HuntcoinUnits::removeSpaces(contextMenuItem->text(COLUMN_AMOUNT)));
}
...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog* dialog)
{
    ...
    int nDisplayUnit = HuntcoinUnits::BTC;
    ...
    l2->setText(HuntcoinUnits::formatWithUnit(nDisplayUnit, nAmount));        // Amount
    l3->setText(HuntcoinUnits::formatWithUnit(nDisplayUnit, nPayFee));        // Fee
    l4->setText(HuntcoinUnits::formatWithUnit(nDisplayUnit, nAfterFee));      // After Fee
    l5->setText(((nBytes > 0) ? ASYMP_UTF8 : "") + QString::number(nBytes));        // Bytes
    l7->setText(fDust ? tr("yes") : tr("no"));                               // Dust
    l8->setText(HuntcoinUnits::formatWithUnit(nDisplayUnit, nChange));        // Change
    if (nPayFee > 0)
    {
    ...
    for (const std::pair<QString, std::vector<COutput>>& coins : mapCoins) {
        ...
        for (const COutput& out : coins.second) {
            ...
            if(ExtractDestination(out.tx->tx->vout[out.i].scriptPubKey, outputAddress))
            {
                ...
                // if listMode or change => show huntcoin address. In tree mode, address is not shown again for direct wallet address outputs
            ...
            // amount
            itemOutput->setText(COLUMN_AMOUNT, HuntcoinUnits::format(nDisplayUnit, out.tx->tx->vout[out.i].nValue));
        ...
        if (treeMode)
        {
            ...
            itemWalletAddress->setText(COLUMN_AMOUNT, HuntcoinUnits::format(nDisplayUnit, nSum));
