...
{
    return QCoreApplication::translate("huntcoin-core", psz).toStdString();
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. huntcoin_de.qm (shortcut "de" needs to be defined in huntcoin.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. huntcoin_de_DE.qm (shortcut "de_DE" needs to be defined in huntcoin.qrc)
...
public:
    ...
    /// Get window identifier of QMainWindow (HuntcoinGUI)
...
#include <qt/huntcoin.moc>

HuntcoinCore::HuntcoinCore():
    QObject()
{
}

void HuntcoinCore::handleRunawayException(const std::exception *e)
{
    PrintExceptionContinue(e, "Runaway exception");
    Q_EMIT runawayException(QString::fromStdString(GetWarnings("gui")));
}

bool HuntcoinCore::baseInitialize()
{
...
void HuntcoinApplication::startThread()
{
    ...
    HuntcoinCore *executor = new HuntcoinCore();
...
void HuntcoinApplication::initializeResult(bool success)
{
    ...
    if(success)
    {
        ...
        if (!vpwallets.empty())
        {
            ...
            window->addWallet(HuntcoinGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(HuntcoinGUI::DEFAULT_WALLET);
        ...
        // huntcoin: URIs or payment requests:
...
int main(int argc, char *argv[])
{
    ...
    /// 6. Determine availability of data directory and parse huntcoin.conf
    ...
    try {
        gArgs.ReadConfigFile(gArgs.GetArg("-conf", HUNTCOIN_CONF_FILENAME));
    } catch (const std::exception& e) {
        QMessageBox::critical(0, QObject::tr(PACKAGE_NAME),
    ...
    if(!masternodeConfig.read(strErr)) {
        QMessageBox::critical(0, QObject::tr("Huntcoin Core"),
    ...
    // huntcoin: links repeatedly have their payment requests routed to this process:
    ...
    try
    {
        ...
        if (HuntcoinCore::baseInitialize()) {
