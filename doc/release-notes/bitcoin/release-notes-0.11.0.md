...
For example, add the following to `huntcoin.conf`:
...
related to the blockchain in the huntcoin system: the raw blocks as received over 
...
For an example script see [huntcoin-submittx](https://github.com/laanwj/huntcoin-submittx).
...
- #5334 `cf87536` libhuntcoinconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new huntcoin-detached-sigs git repo for OSX signatures
...
- #5809 `46bfbe7` Add huntcoin-cli man page
...
- #5849 `41259ca` contrib/init/huntcoind.openrc: Compatibility with previous OpenRC init script variables
