#!/bin/bash
# Copyright (c) 2013 The Bittcoin Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Huntcoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Huntcoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Huntcoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Huntcoin 0.7 datadir."
    ;;
  3)
    echo "Detected Huntcoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected Huntcoin 0.8 datadir."
    ;;
esac
