...
###   huntcoincore.org and huntcoin.org and compares them.
...
WORKINGDIR="/tmp/huntcoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://huntcoincore.org"
HOST2="https://huntcoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="huntcoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'huntcoin-',
...
if [ $? -ne 0 ]; then
   echo "huntcoin.org failed to provide signature file, but huntcoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "huntcoin.org and huntcoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Huntcoin Core binary release signing key installed?"
   fi
