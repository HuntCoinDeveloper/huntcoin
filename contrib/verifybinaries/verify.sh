...
###   huntcoin.africa and huntcoin.africa and compares them.
...
if [ $? -ne 0 ]; then
   echo "huntcoin.africa failed to provide signature file, but huntcoin.africa did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "huntcoin.africa and huntcoin.africa signature files were not equal?"
